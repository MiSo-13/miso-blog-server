...
서버는 사용자가 저장한 최근 블로그 글을 다음 AI 작업의 참고 자료로 자동 사용합니다. 이 기능은 별도 설정 없이 동작합니다.

참고되는 작업:

- 일반 블로그 AI 초안 생성
- 개발 블로그 분석 결과 기반 글 작성
- AI 추가 수정
- AI 품질 리뷰 및 자동 개선

참고 방식:

- 최근 저장 글의 제목, 요약, 태그, 본문 일부를 AI 프롬프트에 넣습니다.
- 이전 글은 새 글의 사실 근거가 아니라 문체, 구성, 태그 흐름을 맞추기 위한 참고자료로만 사용합니다.
- 이전 글의 내용을 그대로 복사하지 말고 톤과 구조만 참고하라고 지시합니다.
- 민감값은 기존 secret masking 규칙으로 가린 뒤 전달합니다.

사용자에게 안내할 문구:

> 저장된 이전 글의 문체와 구성을 참고해서 새 글을 다듬습니다. 이전 글의 사실을 새 글에 섞는 것이 아니라, 블로그 전체의 톤과 구성 일관성을 맞추기 위한 기능입니다.
...
레퍼런스 URL은 블로그 글을 쓸 때 계속 참고하고 싶은 링크를 모아두는 공간입니다. 한 번 저장해두면 이후 AI 초안 생성, AI 추가 수정, AI 품질 리뷰에서 자동으로 참고됩니다.

### 언제 쓰면 좋은가

개발 블로그에서는 다음 링크를 저장하면 좋습니다.

- 공식 문서: Spring, OpenAI, MySQL, GitHub Pages 등
- 장애 원인 분석에 도움이 된 문서
- GitHub Issue, Pull Request, Release Note
- 라이브러리 사용법이나 마이그레이션 가이드
- 내가 자주 참고하는 기술 블로그 글

일반 블로그에서는 다음 링크를 저장하면 좋습니다.
...
AI 호출 시 서버가 저장된 URL의 본문 일부를 실제로 확인합니다. 다만 모든 사이트가 본문을 안정적으로 제공하는 것은 아니므로, `description`에 “이 링크를 왜 참고해야 하는지”를 함께 적어두면 더 좋은 글이 나옵니다.
HTML/Text가 아닌 응답, 로그인/차단 페이지, 접속 실패 페이지는 본문 발췌 없이 실패 사유만 참고됩니다.

좋은 설명 예시:

> Spring Boot 설정 옵션을 확인할 때 참고. 블로그에서는 직접 구현한 설정 흐름을 설명하되, 공식 문서 링크는 참고 링크로만 넣는다.

> 매장 위치와 영업시간 확인용. 맛, 분위기, 방문 경험은 내가 입력한 사진 메모와 본문 메모를 우선한다.
...
프론트에서는 별도 “레퍼런스 관리” 화면을 두는 것을 권장합니다.

1. `개발 레퍼런스`와 `일반 레퍼런스` 탭을 나눕니다.
2. URL, 제목, 설명, 태그, 활성 여부를 입력합니다.
3. 자주 쓰는 링크는 `active=true`로 둡니다.
4. 지금 글에는 참고하지 않았으면 하는 링크는 삭제하지 말고 `active=false`로 잠시 끕니다.
5. AI 초안 생성 화면에서는 “현재 활성 레퍼런스가 함께 참고됩니다” 정도의 안내를 보여줍니다.

### AI가 참고하는 방식

- 일반 블로그 초안 생성은 `GENERAL` 레퍼런스만 참고합니다.
- 개발 블로그 초안 생성은 `DEVELOPMENT` 레퍼런스만 참고합니다.
- 저장된 글을 AI로 수정하거나 품질 개선할 때는 글 종류를 명확히 알기 어려울 수 있어 `GENERAL`, `DEVELOPMENT` 레퍼런스를 모두 참고합니다.
- 활성 레퍼런스 중 최근 항목 일부는 AI 호출 직전에 실제 페이지 제목, 설명, 핵심 문단 발췌를 읽어 프롬프트에 포함합니다.
- 레퍼런스는 사실 검증을 보조하는 자료입니다. 사용자가 입력하지 않은 방문 경험, 가격, 메뉴, 장애 원인, 코드 동작을 만들어내는 근거로 쓰지 않도록 AI 프롬프트에 제한을 걸어두었습니다.
- 품질 리뷰 결과의 `referenceFeedback`, `referenceSentenceSuggestions`에서 레퍼런스 반영 정도와 문장 구조 참고 포인트를 확인할 수 있습니다.

### 사용자 안내 문구 예시

> 레퍼런스 URL은 AI가 글을 쓸 때 참고하는 링크입니다. 서버가 실제 페이지 일부를 읽어 참고하지만, 어떤 점을 참고해야 하는지 설명을 함께 적으면 더 정확합니다.

> 지금 활성화된 레퍼런스 URL이 AI 초안, 수정, 품질 리뷰에 함께 전달됩니다. 이번 글과 관련 없는 링크는 잠시 비활성화해두면 더 정확한 결과를 얻을 수 있습니다.

## 분석 결과 삭제 후 다시 분석하기

GitHub나 로컬 저장소를 분석하다 보면 처음 분석 옵션이 마음에 들지 않거나, 코드가 바뀐 뒤 다시 분석하고 싶을 수 있습니다. 이때 기존 분석 결과를 삭제하고 새로 분석할 수 있습니다.

삭제되는 것은 “분석 결과”입니다. 이미 분석 결과로 만든 블로그 초안은 삭제되지 않습니다.

사용자 화면에서는 다음 흐름을 권장합니다.

1. 분석 결과 목록에서 삭제할 결과를 선택합니다.
2. `분석 결과 삭제`를 누릅니다.
3. 목록이 비워지거나 해당 결과가 사라지면 다시 분석을 실행합니다.
4. 완전히 새로 시작하고 싶다면 `전체 분석 결과 삭제` 후 다시 분석합니다.

사용자 안내 문구 예시:

> 이 작업은 저장소 분석 결과만 삭제합니다. 이미 생성된 블로그 글은 그대로 유지됩니다.

> 코드가 많이 바뀌었거나 더 구체적인 글감을 찾고 싶다면 기존 분석 결과를 삭제한 뒤 `깊게 분석`을 켜고 다시 분석해보세요.

## 깊게 분석하기

`깊게 분석`은 기본적으로 켜져 있습니다. 이 옵션이 켜져 있으면 최근 커밋의 patch뿐 아니라, 변경된 주요 파일의 현재 코드 일부도 함께 참고합니다.

깊게 분석하면 좋아지는 점:

- 단순 커밋 메시지가 아니라 실제 코드 구조까지 보고 글감을 뽑습니다.
- Controller, Service, Entity, Config, Test가 어떻게 연결되는지 더 잘 파악합니다.
- “무엇을 만들었다”보다 “왜 이렇게 설계했는지”, “어떤 문제를 피했는지”를 더 잘 설명할 수 있습니다.
- 블로그 글 후보의 근거 파일이 더 구체적으로 나옵니다.

주의할 점:

- GitHub/OpenAI 분석에서 깊게 분석을 켜면 변경된 코드 일부가 AI 분석 컨텍스트에 포함될 수 있습니다.
- 로컬 저장소의 `LOCAL_ONLY` 분석은 외부 AI로 코드를 보내지 않고 서버 내부에서만 분석합니다.
- 민감한 값은 기존 secret masking 규칙으로 가린 뒤 저장/분석하지만, private 설정 파일이나 key 파일은 커밋에 올리지 않는 것이 가장 안전합니다.

사용자 안내 문구 예시:

> 깊게 분석을 켜면 변경된 주요 파일의 현재 코드 일부까지 참고해서 더 구체적인 블로그 글감을 찾습니다.

> GitHub/OpenAI 분석에서는 코드 일부가 AI 분석에 포함될 수 있습니다. 외부 전송이 부담된다면 로컬 저장소 분석의 `LOCAL_ONLY` 모드를 사용하세요.

## 발행된 글을 다시 수정하기

발행 완료 상태의 글은 실수로 공개 글이 바뀌는 일을 막기 위해 바로 수정되지 않습니다. 이미 `PUBLISHED` 상태가 된 글을 고치려면 먼저 상태를 되돌린 뒤 수정합니다.

추천 흐름:

1. 발행된 글 상세 화면에서 `수정하기`를 누릅니다.
2. “발행된 글을 초안으로 되돌린 뒤 수정합니다” 안내를 확인합니다.
3. 글 상태를 `DRAFT`로 변경합니다.
4. 제목, 요약, 본문, 태그를 수정합니다.
5. 다시 검토, 승인, 발행 흐름을 진행합니다.

사용자 안내 문구 예시:

> 이 글은 이미 발행된 상태입니다. 수정하려면 먼저 초안 상태로 되돌려야 합니다. 이미 GitHub Pages에 올라간 파일은 다시 발행해야 실제 공개 글에도 반영됩니다.

> 상태만 되돌려도 기존 발행 파일이 자동으로 삭제되지는 않습니다. 수정 후 다시 GitHub Pages 발행을 눌러 최신 내용으로 덮어써주세요.
